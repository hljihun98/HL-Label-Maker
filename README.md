...
- **인쇄 중에는 목록을 바꿀 수 없습니다**(`busyPrinting()`). 나눠 보내기는 화면에 묶음만
  올려두고 끝나면 원래 목록으로 되돌리므로, 이때 담은 라벨은 사라지는데 박스 ID 는 이미
  올라가 있습니다 — 라벨 없이 번호만 소모되면 그 번호의 박스를 영원히 찾을 수 없습니다.
- **엑셀은 파일을 그대로 골라 읽습니다**(`xlsx.js`, 첫 번째 시트 · 숨긴 시트는 건너뜁니다).
  파일 이름이 대소문자가 달라도 찾고(`findEntry`), 공유 문자열 표를 못 읽으면 조용히 빈 표를
  만들지 않고 알립니다. CSV 는 한국 윈도우 엑셀의 CP949 저장을 자동으로 되돌립니다.
  한 번에 `SHEET_MAX_ROWS`(5000)행까지만 읽고 넘으면 그 사실을 알립니다. `.xlsx` 는 ZIP 안의 XML 이라
